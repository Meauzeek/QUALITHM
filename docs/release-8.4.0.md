...
后续视觉微调：MET 通过本地 Chroma.js 3.2.0 的 OKLCH 调为更明确的亮蓝紫（目标 L=.59 / C=.20 / h=270°），品牌强调色与 HYP 不变。分别依据日间、夜间基底校准小字对比度至至少 4.5:1，最终为日间 #465fe2、夜间 #516ef2；避免暗蓝或发白的粉蓝。palette.js 统一 CSS 与 JavaScript 的难度色；库未加载时使用同值回退。HYP 定数卡片保持四边等粗，移除额外深色勾线和加厚顶边；缩小折角并加入平行镂空线，日间冰白使用 72% 不透明度透出紫色，夜间保持明亮。斜纹改为粗细、间距交错，并加入柔化边缘。样式与脚本使用新的缓存版本标识，避免发布后继续加载旧外观。
